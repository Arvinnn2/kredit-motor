...
<div class="d-flex justify-content-between align-items-center mb-4">
  <div>
    <h4 class="fw-bold mb-1">Edit Asuransi</h4>
  ...
  <div class="col-lg-6">
    <div class="card" style="border-radius:12px;border:1px solid #e8ecf1;">
      <div class="card-body p-4">
        <form action="{{ route('marketing.asuransi.update', $asuransi) }}" method="POST">
          @csrf @method('PUT')
          <div class="mb-3">
          ...
          <div class="mb-3">
            ...
            <input type="number" step="0.01" name="margin_asuransi" class="form-control @error('margin_asuransi') is-invalid @enderror"
              value="{{ old('margin_asuransi', $asuransi->margin_asuransi) }}">
            @error('margin_asuransi')<div class="invalid-feedback">{{ $message }}</div>@enderror
          </div>
          <div class="mb-4">
            <label class="form-label fw-semibold">Keterangan</label>
            <textarea name="keterangan" class="form-control" rows="3">{{ old('keterangan', $asuransi->keterangan) }}</textarea>
          </div>
